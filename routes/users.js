            ...
            axios
              ...
              .then((verificationResponse) => {
                if (verificationResponse.data.return) {
                  ...
                  res.render("user/forgot-password-verify", {
                    mobile,
                    otp: response.otp,
...
router.post("/forgot-password-otp", (req, res) => {
  ...
  userHelper
    .verifyOTP(otp, token, mobile)
    .then((response) => {
      userHelper
        .forgotPassword(mobile, newPassword)
        .then((response) => {
          res.redirect("/login");
        })
        .catch((err) => {
...
router.post("/signup", (req, res) => {
  ...
  UserHelper.doSignup(req.body).then((response) => {
    ...
    if (response.mobileExist) {
      ...
      req.session.mobileExist =
        "With this mobile number, an account already exists. For more information, please contact the administrator!";
      res.redirect("/signup");
    } else {
      console.log(req.body.mobile);
      userHelper
        .otpsent(req.body.mobile)
        .then((response) => {
          axios
            .post(
              fastsmsapi,
...
router.get("/login", async (req, res) => {
  ...
  if (req.session.user) {
    ...
    res.render("user/login", {
      user: true,
      logedinErr: req.session.logedinErr,
      blockuser: req.session.blockuser,
      allCategory,
    });
...
router.get("/mobile-number", (req, res) => {
  if (req.session.logedIn) {
    ...
    res.render("user/mobile-number", {
      user: true,
      nouser: req.session.noUser,
      accoutBlocked: req.session.accountBlocked,
    });
...
router.get("/products-details-category/:categoryName", async (req, res) => {
  ...
  productBasedOnCategory = await userHelper.productBasedOnCategory(
    categoryName
  );
...
router.get("/view-single-product/:id", async (req, res) => {
  ...
  adminHelper.getProductDetails(req.params.id).then((product) => {
    adminHelper.getRelatedProducts(product.category).then((relatedProducts) => {
      ...
      let firstImageExists = checkImageExists(
        "windows",
        `images/${req.params.id}first.jpg`
      );
      let secondImageExists = checkImageExists(
        "windows",
        `images/${req.params.id}second.jpg`
      );
      let thirdImageExists = checkImageExists(
        "windows",
        `.images/${req.params.id}third.jpg`
      );
      let fourthImageExists = checkImageExists(
        "windows",
        `images/${req.params.id}fourth.jpg`
      );
...
router.get("/remove-product-from-cart/:id/:prodId", (req, res, next) => {
  userHelper
    .deleteProductFromCart(req.params.id, req.params.prodId)
    .then(() => {
      res.redirect("/cart");
    });
});
...
router.get("/order-placed-sucessfully", verifyLogin, async (req, res) => {
  ...
  let allCategory = await adminHelper.getALLCategory();
  let cartCount = 0;
  res.render("user/order-placed-sucessfully", {
    user: true,
    user_login,
    cartCount,
    allCategory,
...
router.get("/otp-verification-contact", async (req, res) => {
  ...
  res.render("user/otp-verification-contact", {
    user: true,
    mobileNumber,
    invalidOtp: req.session.invalidOtp,
  });
...
router.get("/sofasets-beanbags", async function (req, res, next) {
  try {
    let user_login = req.session.user;
    let cartCount = null;
    let allCategory = await adminHelper.getALLCategory();
